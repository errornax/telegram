...
async def start(update: Update, context: CallbackContext) -> None:
    ...
    if update.message:
        await update.message.reply_text("مرحباً بك في بوت يوزر العراقية! اختر الخدمة:", reply_markup=reply_markup)
    else:
        await update.callback_query.edit_message_text("مرحباً بك في الدعم الفني منصة يوزر العراقية! اختر الخدمة:", reply_markup=reply_markup)
...
async def button(update: Update, context: CallbackContext) -> None:
    ...
    if query.data == 'zen_cash':
        ...
        message = "خطوات الدفع عبر فيزا/ماستر:\n1. الرقم: 3658969492\n2. اسم المستلم: Mr. Issa\n3. أرسل صورة التحويل"
        await query.edit_message_text(message, reply_markup=InlineKeyboardMarkup(get_main_menu_button()))
        await forward_to_admin(update, context, "المستخدم اختار: دفع عبر فيزا/ماستر كارد")

    elif query.data == 'report_issue':
        await query.edit_message_text("يرجى كتابة مشكلتك وسيقوم الفريق بالرد عليك قريباً...", reply_markup=InlineKeyboardMarkup(get_main_menu_button()))
        ...
        await query.edit_message_text("أدخل المبلغ بالدولار لتحويله إلى دينار عراقي:", reply_markup=InlineKeyboardMarkup(get_main_menu_button()))
        ...
        await query.edit_message_text(f"سعر الصرف الحالي: 1$ = {DOLLAR_TO_ITUNES}$ iTunes\n\nأدخل المبلغ بالدولار:", reply_markup=InlineKeyboardMarkup(get_main_menu_button()))
...
async def forward_to_admin(update: Update, context: CallbackContext, additional_text=None):
    try:
        ...
        user_info = f"🚀 رسالة جديدة من:\n👤 اسم المستخدم: [{user.full_name}](tg://user?id={user.id})\n🆔 ID: `{user.id}`\n📌 اليوزر: {user_link}\n"
        if additional_text:
        ...
        if update.message and update.message.photo:
            ...
            with open(photo_path, 'rb') as photo_file:
                await context.bot.send_photo(chat_id=MY_CHAT_ID, photo=photo_file, caption=user_info, parse_mode='Markdown')

            await context.bot.send_message(chat_id=user_sessions[user.id], text="تم استلام صورتك بنجاح. يرجى كتابة اليوزر المراد شراؤه:", reply_markup=InlineKeyboardMarkup(get_main_menu_button()))

        elif update.message and update.message.text:
            if user.id in temp_photos:
                user_info += f"\n🎯 اليوزر المطلوب: {update.message.text}\n"
                await context.bot.send_photo(chat_id=MY_CHAT_ID, photo=temp_photos[user.id], caption=user_info, parse_mode='Markdown')
                del temp_photos[user.id]
            else:
                await context.bot.send_message(chat_id=MY_CHAT_ID, text=f"{user_info}\n📩 الرسالة النصية:\n{update.message.text}", parse_mode='Markdown')
...
async def handle_text_input(update: Update, context: CallbackContext) -> None:
    ...
    if user_id in temp_photos:
        ...
        await update.message.reply_text("تم استلام اليوزر بنجاح وسيتم معالجة طلبك قريباً.", reply_markup=InlineKeyboardMarkup(get_main_menu_button()))
        return
    ...
    try:
        ...
        if conversion_type == 'iqd':
            ...
            await update.message.reply_text(f"{amount}$ = {result:,} دينار عراقي", reply_markup=InlineKeyboardMarkup(get_main_menu_button()))

        elif conversion_type == 'itunes':
            result = amount * DOLLAR_TO_ITUNES
            await update.message.reply_text(f"{amount}$ = {result:,.2f}$ رصيد iTunes", reply_markup=InlineKeyboardMarkup(get_main_menu_button()))

        else:
            await update.message.reply_text("⚠️ اختر خدمة أولاً من القائمة", reply_markup=InlineKeyboardMarkup(get_main_menu_button()))

    except ValueError:
        await update.message.reply_text("⚠️ أدخل رقمًا صحيحًا", reply_markup=InlineKeyboardMarkup(get_main_menu_button()))
            ...
            try:
                ...
                if user_id:
                    ...
                    if user_id in user_sessions:
                        await context.bot.send_message(chat_id=user_sessions[user_id], text=f"📬 رد من الدعم الفني:\n\n{reply_text}")
